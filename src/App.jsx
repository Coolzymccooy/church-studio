 ...
 * TIWATON AUDIO AI
 * UPDATE V12.0
 * - Added Voice Activity Detection (VAD) with hysteresis (voice ON/OFF detection)
 * - Gate now respects active speech (less “word chopping”, more natural)
 * - Added VOICE ACTIVE indicator in transport bar
 * - Added Hyper-Gate “How to use” help popover inside Noise Threshold card
 * - Preserved all previous behaviour (live/file, recording, devices, presets)
 */
...
const AudioProcessor = ({ goHome }) => {
  ...
  // Small help popover for Hyper-Gate
  ...
  useEffect(() => {
    ...
    if (!features.denoise && processingRefs.current.gateGain && audioContext) {
      // If gate is off, keep line fully open
      processingRefs.current.gateGain.gain.setValueAtTime(1.0, audioContext.currentTime);
    }
  ...
  const startAudioEngine = async (inputStream = null, fileElement = null) => {
    ...
    try {
      ...
      console.warn('Error creating source node', e);
      return; 
    }
    ...
    visualizeAndGate(setVisualizerGateStatus, setIsVoiceActive); 
  };
  ...
  const toggleLive = async () => {
    if (isLive) { 
      ...
      try {
        const constraints = { 
          audio: { 
            deviceId: selectedDevices.inputId !== 'default' 
  ...
  // --- VISUALIZE + GATE LOGIC (Animation Loop + VAD) ---
  const visualizeAndGate = (setGateStatus, setVoiceStatus) => { 
    ...
    const draw = (timestamp) => {
      ...
      // --- VAD (Voice Activity Detection) with hysteresis ---
      const speechOnDb  = threshold + 10; // must be clearly above noise
      const speechOffDb = threshold + 5;  // drop closer to noise before off

      if (features.denoise && !settingsRef.current.isBypassed) {
        const wasVoiceActive = voiceActiveRef.current;

        if (!wasVoiceActive && db >= speechOnDb) {
          // Voice starts
          voiceActiveRef.current = true;
          silenceFramesRef.current = 0;
        } else if (wasVoiceActive) {
          if (db <= speechOffDb) {
            // Counting quiet frames
            silenceFramesRef.current += 1;
            if (silenceFramesRef.current > 8) {
              // several frames quiet => speech ended
              voiceActiveRef.current = false;
              silenceFramesRef.current = 0;
            }
          } else {
      ...
      // --- Gate Logic (True Gain Clamp + VAD-aware) ---
      if (gateGain) {
        if (features.denoise && !settingsRef.current.isBypassed) {
          const shouldClose = db < threshold && !voiceActiveRef.current;
          const target = shouldClose ? 0.001 : 1.0; 
      ...
      // --- Throttled UI updates (gate + VAD) ---
      if (timestamp - lastUiUpdate > updateInterval) {
        const gateClosed = gateGain ? gateGain.gain.value < 0.1 : false;

        setGateStatus(prev => (prev !== gateClosed ? gateClosed : prev));
        setVoiceStatus(prev => (prev !== voiceActiveRef.current ? voiceActiveRef.current : prev));
        ...
        >
          ...
          <div>
             ...
             <div className="flex items-center gap-1">
               <span className={`w-2 h-2 rounded-full ${isLive || isPlayingFile ? 'bg-green-500 animate-pulse' : 'bg-slate-600'}`}></span>
               <p className="text-[10px] md:text-xs text-slate-400 font-medium">
                 System Ready • v12.0.0
               </p>
             </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
           ...
           <div className="flex bg-slate-900 rounded-full p-1 border border-slate-800">
             <button 
               ...
               className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${mode === 'live' ? 'bg-indigo-600 shadow-lg' : 'text-slate-400 hover:text-white'}`}
             >
               Live Input
             </button>
             <button 
               onClick={() => handleModeSwitch('file')} 
               className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${mode === 'file' ? 'bg-indigo-600 shadow-lg' : 'text-slate-400 hover:text-white'}`}
             >
               File Upload
             </button>
           </div>
           <button 
             ...
             className={`w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-800 transition-colors ${audioStats.sampleRate > 0 && audioStats.sampleRate < 44100 ? 'text-amber-500 animate-pulse' : 'text-slate-400 hover:text-white'}`}
           >
          ...
          <div className="bg-slate-900 w-full max-w-lg rounded-2xl border border-slate-700 shadow-2xl p-6 space-y-6">
             ...
             <div>
               ...
               <select 
                 ...
                 onChange={(e) => setSelectedDevices({...selectedDevices, inputId: e.target.value})}
               >
             ...
             <div>
               ...
               <select 
                 ...
                 onChange={(e) => setSelectedDevices({...selectedDevices, outputId: e.target.value})}
               >
             ...
             <div className="grid grid-cols-2 gap-4">
               <div className={`p-4 rounded-xl border ${audioStats.sampleRate > 0 && audioStats.sampleRate < 44100 ? 'bg-amber-900/20 border-amber-600/50' : 'bg-slate-800 border-slate-700'}`}>
                 <div className="text-xs font-bold mb-1">
                   SAMPLE RATE
                 </div>
                 <div className={`text-lg font-mono flex items-center gap-2 ${audioStats.sampleRate > 0 && audioStats.sampleRate < 44100 ? 'text-amber-500' : 'text-indigo-400'}`}>
                   {audioStats.sampleRate > 0 ? (audioStats.sampleRate / 1000).toFixed(1) + ' kHz' : '--'}
                   {audioStats.sampleRate > 0 && audioStats.sampleRate < 44100 && <AlertTriangle size={16} />}
                 </div>
               </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 pb-20 lg:pb-6">
            ...
            <div className="mb-8 space-y-3">
              ...
              <FeatureToggle 
                ...
                desc="Blocks typing, fans & low-level room noise" 
                active={features.denoise} 
                onClick={() => setFeatures({...features, denoise: !features.denoise})} 
              />

              {/* Hyper-Gate Detail & Help */}
              {features.denoise && (
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 mt-3 -mb-3 transition-opacity duration-300 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <div className="flex items-center gap-2">
                        {visualizerGateStatus 
                          ? <span className="text-[9px] font-bold text-red-500 tracking-wider animate-pulse">GATE CLOSED</span> 
                          : <span className="text-[9px] font-bold text-green-500 tracking-wider">GATE OPEN</span>
                        }
                        <span className={`text-xs font-mono ${noiseFloorThreshold > -40 ? 'text-red-400' : 'text-indigo-400'}`}>
                          {noiseFloorThreshold} dB
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-[10px] text-slate-500">
                        <span className={`w-2 h-2 rounded-full ${isVoiceActive ? 'bg-emerald-400 animate-pulse' : 'bg-slate-600'}`}></span>
                        <span>
                          {isVoiceActive ? 'Pastor speech detected above noise' : 'Waiting for speech above noise floor'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setNoiseFloorThreshold(parseFloat(e.target.value))} 
                    className="w-full h-1 bg-indigo-700 rounded-lg appearance-none cursor-pointer" 
                  />
                  
                  <p className="text-[10px] text-slate-500 mt-2 leading-tight">
                    Slide until the room noise disappears, but your voice still lights up “Pastor speech detected”.
                  </p>

                  {/* Tiny help popover trigger */}
                  <button
                    type="button"
                    onClick={() => setShowGateHelp(prev => !prev)}
                    className="mt-3 inline-flex items-center gap-1 text-[10px] font-semibold text-indigo-300 hover:text-indigo-200"
                  >
                    <AlertCircle size={12} />
                    How to use Hyper-Gate
                  </button>

                  {showGateHelp && (
                    <div className="mt-2 text-[10px] text-slate-300 bg-slate-950/70 border border-slate-700 rounded-lg p-3 space-y-1">
                      <p className="font-semibold text-indigo-300 mb-1">
                        Quick setup (for volunteers):
                      </p>
                      <ol className="list-decimal list-inside space-y-1">
                        <li>Leave the room noisy (fans, chairs, quiet chatting) and keep the pastor silent.</li>
                        <li>Move the slider right until the gate says <span className="text-red-400 font-semibold">GATE CLOSED</span> and the meters are calm.</li>
                        <li>Ask the pastor to speak normally into the mic – “Pastor speech detected” should light up in green.</li>
                        <li>If words sound cut off, move the slider a little back to the left.</li>
                      </ol>
                      <p className="mt-1">
                        Once this is set, you shouldn’t touch it again unless the room changes (new hall, different background noise).
                      </p>
                    </div>
                  )}
                </div>
              ...
              <FeatureToggle 
                ...
                desc="Softens big hall reflections (light treatment)" 
                active={features.dereverb} 
                onClick={() => setFeatures({...features, dereverb: !features.dereverb})} 
              />
              <FeatureToggle 
                icon={<Mic size={18} />} 
                label="Pastor Voice Isolation" 
                desc="Boosts clarity band so words cut through the mix" 
                active={features.pastorIsolation} 
                onClick={() => setFeatures({...features, pastorIsolation: !features.pastorIsolation})} 
              />
              <FeatureToggle 
                icon={<Speaker size={18} />} 
                label="Sermon Warmth" 
                desc="Adds body like a good podcast preamp" 
                active={features.sermonWarmth} 
                onClick={() => setFeatures({...features, sermonWarmth: !features.sermonWarmth})} 
              />
              <FeatureToggle 
                icon={<Sliders size={18} />} 
                label="Smart Auto-Mixing" 
                desc="Tames shouty peaks, keeps level steady for stream" 
                active={features.smartMixing} 
                onClick={() => setFeatures({...features, smartMixing: !features.smartMixing})} 
              />
            </div>
            ...
            <FeatureToggle 
              ...
              desc="-14 LUFS style glue for online platforms" 
              active={features.mastering} 
              onClick={() => setFeatures({...features, mastering: !features.mastering})} 
            />
        ...
        <div className="flex-1 flex flex-col relative bg-slate-900 h-full overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-black/60 z-30 backdrop-blur-sm px-4">
              <div className="text-center p-8 bg-slate-800 rounded-2xl border border-slate-700 shadow-2xl w-full max-w-md">
                ...
                <p className="text-slate-400 mb-6">
                  Connect your mixer or mic to calibrate the AI engine.
                </p>
                <button 
                  onClick={toggleLive} 
                ...
                {recordingState === 'review' && (
                  <div className="flex items-center">
                    ...
                    <button 
                      ...
                      disabled={exportStatus === 'sharing' || exportStatus === 'done'} 
                      className={`px-4 py-2 hover:bg-slate-700 rounded-r-full font-bold text-sm flex items-center gap-2 ${exportStatus === 'done' ? 'text-green-400' : 'text-indigo-400'}`} 
                      title="Share / Export"
                    >
          ...
          <div className="flex-1 relative overflow-hidden bg-slate-900 w-full h-full">
            ...
            <div className="hidden lg:flex absolute top-4 right-4 gap-2 z-10">
              ...
              <Badge active={features.pastorIsolation && !isBypassed} text="VOICE ISOLATION" />
            ...
            <canvas 
              ...
              width={1000} 
              height={500} 
              className={`w-full h-full object-cover transition-opacity ${isBypassed ? 'opacity-40 grayscale' : 'opacity-80'}`} 
            />
          </div>
          ...
          <div className="flex-none h-20 md:h-24 bg-slate-950 border-t border-slate-800 px-4 md:px-8 flex items-center justify-between z-20">
            ...
            <div className="flex items-center gap-4 md:gap-6">
              <button 
                ...
                className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center transition-all ${isLive ? 'bg-red-500 hover:bg-red-600 shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-indigo-600 hover:bg-indigo-500 shadow-[0_0_20px_rgba(79,70,229,0.4)]'}`}
              >
                {isLive 
              ...
              <div className="hidden md:flex flex-col gap-1">
                ...
                <div className={`text-sm font-medium flex items-center gap-2 ${isLive ? 'text-green-400' : 'text-slate-400'}`}>
                  {isLive && recordingState === 'review' 
                    ? <span className="text-amber-400">Reviewing (Mic Muted)</span> 
                    : isLive 
                      ? (<><span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"/> Live Processing</>) 
                      : 'Standby'}
                </div>
              </div>
              ...
              <div className="hidden md:flex items-center">
                <span className={`px-2 py-1 rounded-full text-[10px] font-bold tracking-wider border
                  ${isVoiceActive 
                    ? 'bg-emerald-900/40 text-emerald-300 border-emerald-500/40' 
                    : 'bg-slate-800 text-slate-500 border-slate-700'}`}>
                  {isVoiceActive ? 'VOICE ACTIVE' : 'NO VOICE DETECTED'}
                </span>
              </div>
            </div>
            ...
            {/* Right: Bypass / Monitor / Target */}
            <div className="flex items-center gap-2 md:gap-4">
              <button 
                onClick={() => setIsBypassed(!isBypassed)} 
                className={`p-2 md:p-3 rounded-lg border flex gap-2 text-xs font-bold ${isBypassed ? 'bg-red-900/50 border-red-500 text-red-200' : 'bg-slate-800 border-slate-700 text-slate-400'}`}
              >
                <Power size={18} /> 
              ...
              <button 
                ...
                className={`p-2 md:p-3 rounded-lg border relative ${isMonitoring ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'} ${recordingState === 'review' ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                <Headphones size={20} />
              ...
              <button 
                ...
                className="hidden md:flex items-center gap-2 p-3 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-300 border border-slate-700" 
                title="Virtual routing target (visual only, configure routing in your streaming software)"
              >
                {outputTargets.find(t => t.name === outputTarget)?.icon || <Cpu size={20} />}
                <span className="text-xs font-medium text-slate-400">
                  {outputTarget}
                </span>
              </button>
...
const FeatureToggle = ({ icon, label, desc, active, onClick }) => (
  <div 
    onClick={onClick} 
    className={`p-3 rounded-xl border cursor-pointer transition-all group ${active ? 'bg-indigo-900/30 border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.2)]' : 'bg-slate-900 border-slate-800 hover:border-slate-700 opacity-60 hover:opacity-100'}`}
  >
    <div className="flex items-center justify-between mb-1">
      <div className={`flex items-center gap-3 ${active ? 'text-indigo-400' : 'text-slate-400 group-hover:text-slate-300'}`}>
        {icon}
        <span className="font-semibold text-sm">{label}</span>
      </div>
      <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${active ? 'bg-indigo-500 border-indigo-500' : 'border-slate-600'}`}>
  ...
  <div className="flex flex-col items-center gap-1">
    <div className="flex items-end gap-0.5 h-6 md:h-10">
      {[...Array(5)].map((_, i) => (
        <div 
          key={i} 
          className={`w-1 md:w-1.5 rounded-sm transition-all ${level > (i * 20) ? color : 'bg-slate-800'}`} 
          style={{ height: `${(i + 1) * 20}%` }} 
        />
      ))}
    </div>
...
const Badge = ({ active, text }) => (
  <div className={`px-2 py-1 rounded text-[10px] font-bold tracking-wider border ${active ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-slate-800 text-slate-600 border-transparent'}`}>
    {text}
  </div>
);
